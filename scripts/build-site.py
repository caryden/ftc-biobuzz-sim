...
def nav(current):
    ...
    return '<nav class="sitenav" aria-label="Site"><a class="brand" href="/">NCSSM FTC · BIOBUZZ</a>' + ''.join('<a href="%s"%s>%s</a>' % (h, ' aria-current="page"' if h == current else '', t) for h, t in items[1:]) + UNITS_SWITCH + '</nav>' + UNITS_SCRIPT
