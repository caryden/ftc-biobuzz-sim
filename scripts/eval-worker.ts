...
// intake on, where it starts off (see `AUTO_TUNING.intakeDefault`). NO_TURRET_INTAKE_HEADING=1 turns off
// `EXEC.turretIntakeHeading`.
...
if (process.env.NO_TURRET_INTAKE_HEADING) EXEC.turretIntakeHeading = false; // A turret robot keeps its heading on the way to launch.
