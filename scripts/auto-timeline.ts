...
while (sim.phase !== 'post') {
  ...
  for (const k of [0, 1]) { const n = coaches[k].auto?.note ?? ''; if (n === last[k]) continue; last[k] = n; const p = sim.robots[k].body.translation(); console.log(`${t.toFixed(1).padStart(5)} s R${k} field (${p.x.toFixed(2)}, ${(-p.z).toFixed(2)}) [${sim.robots[k].carried.map(code)}] ${n}`); }
